  ...
  return (
    <div className="relative w-full h-screen">
      ...
      <div className="absolute top-0 left-0 w-full text-center text-xs font-mono tracking-widest p-2 uppercase">
        FREE SHIPPING TO RWANDA ON ALL ORDERS ABOVE
      </div>

      {/* Navbar */}
      <nav className="absolute top-4 left-6 right-6 flex justify-between items-center">
        <div className="flex mt-10 gap-x-6">
          <button className="p-4 bg-lightblue3 rounded-full" onClick={() => { setIsMenuOpen(true); setIsCartOpen(false); }}>
            <IoMenu className="text-white"/>
          </button>
          <h1 className="text-3xl font-bold tracking-tight "style={{marginTop:5}}>SING<span className="text-lightblue4">IZA</span></h1>
        </div>

        <div className="flex gap-4">
          <button className="p-4 bg-lightblue3 rounded-full mt-12">
            <CiSearch size={20} className="text-white"/>
          </button>
          <button className="p-4 bg-lightblue3 rounded-full mt-12" onClick={() => { setIsCartOpen(true); setIsMenuOpen(false); }}>
      ...
      <div className="absolute bottom-10 left-20 text-white max-w-xl">
        <h2 className="text-4xl font-bold"> Warm<span className="text-lightblue4"> & Cozy</span></h2>
        ...
        <div className="mt-6 flex justify-start">
          ...
          className="px-10 py-2 bg-lightblue3 text-white font-semibold rounded-lg hover:bg-lightblue3 hover:text-white transition duration-300 ease-in-out">
            Shop Hoodies Now
          </button>
        </div>
      </div>
      ...
      {isMenuOpen && (
        <div className="fixed top-0 left-0 w-80 h-full bg-gray-700 bg-opacity-90 backdrop-blur-md p-6 text-white shadow-lg transform transition-transform duration-300 ease-in-out z-50">
          ...
          <div className="mt-6 space-y-2">
            ...
            <button onClick={() => setExpandedAccounts(!expandedAccounts)} className="flex justify-between w-full border-b border-lightblue4 pb-2">
              Accounts {expandedAccounts ? "-" : "+"}
            </button>
            {expandedAccounts && (
              <ul className="pl-4 mt-2 space-y-2">
                <li><button onClick={() => navigate("/admin-page")} className="text-white">Admin</button></li>
              </ul>
            )}
          </div>
        </div>

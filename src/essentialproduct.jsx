  ...
  return (
    <div className="p-20 px-20 bg-gray-50"> 
      <div className="flex flex-col items-start mb-4">
        <h1 className="font-bold text-3xl ml-8">Shop <span className="text-blue-500">Best</span> Socks</h1>
        <select className="mt-2 p-2 border rounded-md self-end bg-blue-200 w-52 border-blue-400">
      ...
      <div className="flex gap-10"> 
        <div className="w-1/4 pr-4 max-h-[400px] overflow-y-auto"> 
          <h2 className="mb-10 font-bold text-3xl">FILT<span className="text-blue-500">ERS</span></h2>
          {Object.keys(filters).map((key) => (
            <div key={key} className="mb-2 border-b pb-2">
              <div className="flex justify-between items-center cursor-pointer" onClick={() => toggleFilter(key)}>
                ...
                {filters[key] ? <FaMinus className="text-blue-400"/> : <FaPlus className="text-blue-400"/>}
              </div>
              {filters[key] && (
            ...
            <div key={index} className="border p-4 rounded-md flex flex-col items-center bg-white shadow-md">
              ...
              <div className="flex space-x-2 mt-2">
                {product.colors.map(color => (
                  <div key={color} style={{ backgroundColor: colorMap[color] }} className="w-5 h-5 rounded-full border border-gray-300"></div>
                ))}
              </div>
            </div>
          ))}
